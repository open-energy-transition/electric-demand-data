...
def test_save_time_series(sample_time_series, tmp_path):
    # Define the file path where the time series will be saved.
    file_path = os.path.join(tmp_path, "test_output.csv")

    # Save the time series to a csv.
    save_time_series(sample_time_series, file_path, "TestVariable", local_time_zone)
