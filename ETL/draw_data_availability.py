...
def get_year_fractions(codes: list[str]) -> dict[str, dict[str, float]]:
    """
    Get the fractions of years for which data is available for each country or subdivision.

    Parameters
    ----------
    codes : list[str]
        A list of ISO alpha-2 or a combination of ISO alpha-2 and subdivision codes
...
def retrieve_ember_electricity_data() -> pandas.DataFrame:
    """
...
def retrieve_world_bank_data(variable: str) -> pandas.DataFrame:
    """
    ...
    # Get the name of data file in the archive. It is the file that does not start with "Metadata" and ends with ".csv".
...
def extract_ember_electricity_data(
...
def extract_world_bank_data(
    ...
    """
    Extract the data for a specific country and for the years of interest from the World Bank dataset.
    ...
    # Extract the values for the available years in the World Bank dataset.
...
def get_electricity_demand_data(
    ...
    """
    Get the electricity demand data for a specific country and for the years of interest.
    ...
    years_of_interest : dict[str, list[str]]
        A dictionary where the keys are entitiy codes and the values are lists of strings representing the years of interest
    ...
    world_bank_electricity_data = retrieve_world_bank_data(
        "electricity_demand_per_capita"
    )

    # Initialize the electricity demand data series for each country or subdivision.
    # The dictionary structure is specified bacause required by the type hint.
    electricity_demand_data: dict[str, dict[str, pandas.Series]] = {}

    # Loop over the ISO alpha-3 codes.
    for code in codes:
        # Get the electricity demand data for the country and for the available years in the World Bank dataset.
        world_bank_series = extract_world_bank_data(
            world_bank_electricity_data,
            alpha_3_codes[code],
            years_of_interest[code],
        )

        # Get the electricity demand data for the country and for the available years in the Ember dataset.
        ember_series = extract_ember_electricity_data(
            ember_electricity_data,
            alpha_3_codes[code],
            years_of_interest[code],
        )

        # Combine the World Bank and Ember datasets, with an average of the two datasets for each year if both are available.
        combined_series = (world_bank_series + ember_series) / 2

        # Where the combined series is NaN because one of the datasets is missing, use the other dataset.
        combined_series = combined_series.fillna(world_bank_series).fillna(ember_series)

        # If the country is already in the dictionary, add a new key for the code.
        if alpha_3_codes[code] in electricity_demand_data:
            electricity_demand_data[alpha_3_codes[code]][code] = combined_series
        else:
...
def get_world_bank_gdp_data(
    ...
    """
    Get the GDP per capita data for a specific country and for the years of interest.
    ...
    # Loop over the ISO alpha-3 codes.
    for code in codes:
        # Get the GDP data for the country and for the available years in the World Bank dataset.
        gdp_series = extract_world_bank_data(
            world_bank_gdp_data,
            alpha_3_codes[code],
            years_of_interest[code],
        )
...
def get_occurrences(
    ...
    """
    Get the occurrences of electricity demand or GDP per capita data in the defined levels and by continent.

    Parameters
    ----------
    data : dict[str, pandas.Series]
        A dictionary where the keys are entity codes and the values are pandas Series with years as index and values as electricity demand or GDP per capita
    codes : list[str]
        A list of ISO alpha-2 or a combination of ISO alpha-2 and subdivision codes
    alpha_3_codes : dict[str, str]
        A dictionary where the keys are entity codes and the values are ISO alpha-3 codes
    continent_codes : dict[str, str]
        A dictionary where the keys are entity codes and the values are continent codes
    fractions_of_years : dict[str, dict[str, float]]
        A dictionary where the keys are entity codes and the values are dictionaries with years as keys and fractions of years as values
    levels : dict[str, tuple[float, float]]
        A dictionary where the keys are level names and the values are tuples with minimum and maximum values for the level
    ...
    for code in codes:
        ...
        for year in data[alpha_3_codes[code]][code].index:
            # Loop over the levels and continents and add the occurrence to the corresponding level and continent.
            for level, (min_val, max_val) in levels.items():
                if min_val <= data[alpha_3_codes[code]][code][year] < max_val:
                    occurrence[continent_codes[code]][level] += fractions_of_years[
                        code
                    ][year]
...
def add_bar_chart(
    ...
    """
    Add a bar chart to the given axes with the occurrences of electricity demand or GDP per capita data in the defined levels and by continent.
    ...
    levels : dict[str, tuple[float, float]]
        A dictionary where the keys are level names and the values are tuples with minimum and maximum values for the level
    occurrence : dict[str, dict[str, float]]
        A dictionary where the keys are continent codes and the values are dictionaries with levels as keys and occurrences as values
    xlabel : str
    ...
    for continent_code in occurrence.keys():
        ...
        cumulative_height += numpy.array(list(occurrence[continent_code].values()))
...
# Get the fractions of years for which data is available for each country or subdivision.
fractions_of_years = get_year_fractions(codes)

# Extract the available years for each country or subdivision.
available_years = {code: list(fractions_of_years[code].keys()) for code in codes}

# Get the electricity demand data from Ember and the World Bank.
electricity_data = get_electricity_demand_data(
    codes,
    alpha_3_codes,
    available_years,
)
...
gdp_data = get_world_bank_gdp_data(
    codes,
    alpha_3_codes,
    available_years,
)
...
# Get the electricity demand occurrence in the defined electricity demand levels and by continent.
electricity_demand_occurrence = get_occurrences(
    electricity_data,
    codes,
    alpha_3_codes,
    continent_codes,
...
gdp_occurrence = get_occurrences(
    gdp_data,
    codes,
    alpha_3_codes,
    continent_codes,
