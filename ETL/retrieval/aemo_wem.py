...
def get_available_requests() -> list[tuple[bool, int, int | None, int | None]]:
    ...
    # Return the available requests that combine the two lists and add a boolean flag to indicate if the request is for the pre-reform period.
...
def get_url(pre_reform: bool, year: int, month: int | None, day: int | None) -> str:
    """
    ...
    if pre_reform:
        ...
        return f"https://data.wa.aemo.com.au/datafiles/operational-demand/operational-demand-{year}.csv"

    else:
        # If the request is for the post-reform period, set the URL to fetch .json files for data from September 2023 onward
        return f"https://data.wa.aemo.com.au/public/market-data/wemde/operationalDemandWithdrawal/dailyFiles/OperationalDemandAndWithdrawal_{year}-{month:02d}-{day:02d}.json"
