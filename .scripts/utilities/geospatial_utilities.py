...
def remove_remote_islands(region_shape, country_code):
    """
    ...
    if country_code in ["CL", "ES", "FR", "NL", "NO", "NZ", "PT"]:
...
def get_geopandas_region(country_code, make_plot=True):
    """
    ...
    try:
        try:
            ...
            region_shape = [
                ...
                if ii.attributes["ISO_A2"] == country_code
            ][0]
        except IndexError:
        ...
        # Remove small remote islands from the shape of some countries.
        region_shape = remove_remote_islands(region_shape, country_code)

        # Add the ISO Alpha-2 code as the name of the region.
        region_shape["name"] = country_code
        region_shape = region_shape.set_index("name")

        if make_plot:
            figure_utilities.plot(region_shape, f"region_shape_{country_code}")

    except (AttributeError, IndexError):
        print(f"Region shape for {country_code} not found.")
        region_shape = None

    return region_shape


def get_region_bounds(region_shape):
    """
...
def load_xarray(file_name, engine="netcdf4", dataarray_or_dataset="dataarray"):
    """
    ...
    ----------
    file_name : str
        The name of the file to load the xarray dataset from
    engine : str
    ...
    if dataarray_or_dataset == "dataarray":
        xarray_dataset = xr.open_dataarray(file_name, engine=engine)
    elif dataarray_or_dataset == "dataset":
        xarray_dataset = xr.open_dataset(file_name, engine=engine)

    # Harmonize the coordinates of the xarray dataset.
    xarray_dataset = harmonize_coords(xarray_dataset)
